...
class BatchManager {
    ...
    /**
     * Create a new batch when restocking or adjusting stock
     */
     ...
     */
    public function getAvailableBatches($product_id, $quantity_needed = null) {
        $sql = "
     ...
     */
    public function consumeStock($product_id, $quantity, $movement_type, $reference_type = null, $reference_id = null, $created_by = null, $notes = null) {
        // Check if there's already an active transaction
        $has_transaction = $this->pdo->inTransaction();
        ...
        try {
            ...
            $batches = $this->getAvailableBatches($product_id);
            ...
            if ($remaining_quantity > 0) {
                throw new Exception("Insufficient stock. Need {$quantity}, but only " . ($quantity - $remaining_quantity) . " available.");
            }
