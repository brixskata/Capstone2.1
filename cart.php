...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($product_id > 0) {
        ...
        if (empty($cart_data['items']) && !empty($_SESSION['cart'])) {
            ...
            // Find the item in the calculated items
            foreach ($cart_data['items'] as $cart_key => $item) {
                if ($item['id'] == $product_id || (is_string($cart_key) && strpos($cart_key, $product_id . '_') === 0)) {
        ...
        function updateQuantity(productId, action) {
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    if (qtySpan) {
                        console.log('Current quantity:', qtySpan.textContent, 'New quantity:', data.quantity);
                    ...
                    // Update cart total in order summary with animation
