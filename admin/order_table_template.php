           ...
           <tr class="order-row" style="transition: all 0.2s ease;">
            ...
            <td>
              <div class="d-flex gap-1 flex-wrap">
                <!-- View Details Button -->
                <button type="button" class="action-btn" style="background: #6c757d; color: white;" onclick="event.stopPropagation(); viewOrderDetails(<?= $order['id'] ?>, '<?= addslashes($order['username']) ?>', '<?= addslashes($order['email'] ?? '') ?>', '<?= addslashes($order['phone'] ?? '') ?>', '<?= addslashes($order['address_line'] ?? '') ?>', '<?= addslashes($order['address_line2'] ?? '') ?>', '<?= addslashes($order['city'] ?? '') ?>', '<?= addslashes($order['state'] ?? '') ?>', '<?= addslashes($order['postal_code'] ?? '') ?>', '<?= addslashes($order['country'] ?? '') ?>', '<?= addslashes($order['items']) ?>', '<?= $order['total_amount'] ?>', '<?= addslashes($order['payment_method'] ?? '') ?>', '<?= addslashes($order['payment_proof'] ?? '') ?>', '<?= addslashes($order['gcash_transaction_id'] ?? '') ?>', '<?= addslashes($order['status']) ?>', '<?= addslashes($order['delivery_option'] ?? '') ?>', '<?= $order['created_at'] ?>', '<?= addslashes($order['application_name'] ?? '') ?>', '<?= addslashes($order['rider_name'] ?? '') ?>', '<?= addslashes($order['plate_number'] ?? '') ?>', '<?= addslashes($order['transaction_number'] ?? '') ?>')">
